...
export class TxServices {
    async callExtensionContract(walletId: `${string}.${string}`, params: ExtensionCallParams) {
        const delegateAmount = (+params["amount-ustx"] * Math.pow(10, params.decimal))
    ...
    async sendTransaction(
        ...
        if (params.assetType === "nft") {
            ...
            if (params.asset === "stx") {
                txConditions = [Pc.principal(params.from).willSendLte(params.amount).ustx()]
                txOption = {
                    contract: `${cswAddress}.${cswName}`,
                    functionName: "stx-transfer",
                    functionArgs: [Cl.uint(params.amount), Cl.principal(params.to), Cl.none()],
                    postConditions: txConditions
                }
            } else {
                ...
                txOption = {
                    ...
                    functionArgs: [Cl.uint(+params.amount), Cl.principal(params.to), Cl.none(), Cl.contractPrincipal(assetAddress.split('.')[0], assetAddress.split('.')[1])],
                    postConditions: txConditions
                }
            }
        }
